...
doc = fitz.open('/app/filled_LJK.xlsx.pdf')
page = doc[0]
pix = page.get_pixmap(matrix=fitz.Matrix(2.0, 2.0))
arr = np.frombuffer(pix.tobytes('png'), np.uint8)
bgr = cv2.imdecode(arr, cv2.IMREAD_COLOR)
warped, method, *_ = detect_corners_and_crop(bgr)
...
for col_name in ['Soal-A', 'Soal-B', 'Soal-C', 'Soal-D', 'Soal-E']:
    ...
    y_min = int(min(y_vals)) - 60
    y_max = int(max(y_vals)) + 60

    sub = g[y_min:y_max, x_min:x_max]
    _, bw = cv2.threshold(sub, 160, 255, cv2.THRESH_BINARY_INV)
    ...
    rows = []
    for b in boxes:
        if not rows or abs(b[1] - rows[-1][0][1]) > 15:
            rows.append([b])
        else:
            rows[-1].append(b)

    print('%s: %d rows (expected 15)' % (col_name, len(rows)))
    for i, row in enumerate(rows[:15]):
        y_avg = sum(b[1] for b in row) / len(row)
        xs_r = sorted([b[0] for b in row])
        print('  row %d: y=%.0f n=%d xs=%s' % (i, y_avg, len(row), [int(x) for x in xs_r]))